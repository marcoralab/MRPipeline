...
params: 
  ...
  dbp.summary: ~/Dropbox/Research/Data/Summary_Statisitics/BP/Vangelis2018/UKB-ICBPmeta750k_DBPsummaryResults.txt.gz
  pp.summary: ~/Dropbox/Research/Data/Summary_Statisitics/BP/Vangelis2018/UKB-ICBPmeta750k_PPsummaryResults.txt.gz
  sbp.summary: ~/Dropbox/Research/Data/Summary_Statisitics/BP/Vangelis2018/UKB-ICBPmeta750k_SBPsummaryResults.txt.gz
...
write_tsv(hipv.dat, gzfile('~/Dropbox/Research/PostDoc-MSSM/2_MR/1_RawData/GWAS/hipv2015_GWAS.Processed.gz'))
```
...
[Doherty et al Nat Commun 2018](https://www.nature.com/articles/s41467-018-07743-4): 
...
```{r data_wrangling_exposure, echo=F, cache=TRUE}
sleepdoherty.raw <- read_csv('~/Dropbox/Research/Data/Summary_Statisitics/Sleep/Doherty2018/Doherty-2018-NatureComms-sleep-duration-SexBMI.csv.gz')
sleepdoherty <- sleepdoherty.raw %>% 
  rename(POS = BP, Effect_allele = ALLELE1, Non_Effect_allele = ALLELE0, EAF = A1FREQ, Beta = BETA, P = P_LINREG) %>% 
  filter(nchar(Effect_allele) == 1 & Effect_allele %in% c('A', 'C', 'T', 'G')) %>% 
  filter(nchar(Non_Effect_allele) == 1 & Non_Effect_allele %in% c('A', 'C', 'T', 'G')) %>% 
  filter(CHR %in% 1:22) %>%
  arrange(CHR, POS) %>% 
  mutate(N = 91105) %>%
  mutate(r2 = snp.r2(EAF, Beta)) %>%
  filter(SNP %nin% c("rs552290972", "rs2693333", "rs2696685", "rs553682853")) %>%
  select(SNP, CHR, POS, Effect_allele, Non_Effect_allele, EAF, Beta, SE, P, r2, N)

write_tsv(sleepdoherty, gzfile('~/Dropbox/Research/PostDoc-MSSM/2_MR/1_RawData/GWAS/sleepdoherty_GWAS.Processed.gz'))
