...
params: 
  ...
  hipv_2015.summary: '~/LOAD_minerva/dummy/Data/GwasSummaryStatistics/Hippocampal_Volume/Stein2015/ENIGMA2_MeanHippocampus_Combined_GenomeControlled_Jan23.tbl.gz'
...
### Hippocampal Volume - Stein 2015

```{r}
hipv.raw <- read_table2(params$hipv_2015.summary)
hipv.dat <- hipv.raw %>%
...
write_tsv(hipv.dat, gzfile('~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/hipv2015_GWAS.Processed.gz'))
```
...
ggsave('~/Dropbox/Research/PostDoc-MSSM/2_MR/4_Output/hipv2015.png', device = 'png', units = 'in', width = 10, height = 5, plot = out.hipv)
```
...
write_tsv(cpd, gzfile('~/Dropbox/Research/PostDoc-MSSM/2_MR/2_DerivedData/cpd_GWAS.Processed.gz'))

```

```{r}
## Number of Significant SNPs
cpd_ld <- format_data(filter(cpd, P < 5e-5), type = 'exposure', 
                      phenotype_col = 'cpd',
                      snp_col = 'SNP', 
                      beta_col = "Beta",
                      se_col = "SE",
                      ...
                      other_allele_col = "Non_Effect_allele",
                      pval_col = "P")
cpd_ld <- clump_data(cpd_ld, clump_r2 = 0.1, clump_kb = 250)

clump.n <- cpd_ld %>% 
  mutate(p.cut = cut(pval.exposure, breaks = 
                       c(Inf, 0.05, 5e-5, 5e-6, 5e-7, 5e-8, -Inf), lablels = F)) %>%
  left_join(select(cpd, SNP, r2)) %>% 
  select(SNP, effect_allele.exposure, other_allele.exposure, eaf.exposure, beta.exposure, se.exposure, p.cut, r2) %>% 
  group_by(p.cut) %>% summarise(n = n(), r2 = round(sum(r2, na.rm = T), 4))
...
tab.cpd <- full_join(all.n, clump.n, by = 'p.cut', suffix = c('.all', '.clump'))
```

```{r alc_cons gwas}

p.cpd <- ggman(filter(cpd, P < 0.05), snp = 'SNP', chrom = 'CHR', bp = 'POS', pvalue = 'P', ymin = 0, ymax = 60, 
               title = "cpd - AUTHOR") + 
  theme_classic() + 
  geom_hline(yintercept = -log10(5e-5), colour = 'blue', size = 0.25) + 
  geom_hline(yintercept = -log10(5e-6), colour = 'red', size = 0.25, linetype = 2)

out.cpd <- grid.arrange(p.cpd,  tableGrob(tab.cpd, theme = ttheme_minimal(base_size = 8)), 
                        ncol = 2, widths = 2:1, layout_matrix = rbind(c(1,2), c(1, NA)))

ggsave('~/Dropbox/Research/PostDoc-MSSM/2_MR/4_Output/plots/cpd.png', device = 'png', units = 'in', width = 10, height = 5, plot = out.cpd)
